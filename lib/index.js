...
var watch = function (path, args) {
    ...
    if (typeof path === "undefined" || path === null) {
        console.error("\u001b[91mNo filepath specified\u001b[0m");
        process.exitCode = 1;
        return;
    }

    // Backwards compatbility
    if (Array.isArray(argv._) && argv._.length > 0  && !(Object.keys(argv).some(r => ['mode','suffix','scale','intp'].indexOf(r) > -1))){
        argv.mode = 'add';
        argv.suffix = argv._[0];
        if (argv._.length > 1){
            argv.scale = argv._[1];
        }
        if (argv._.length > 2){
    ...
    if (argv.hasOwnProperty('mode')) {
        ...
        if (tempMode !== 'sub' && tempMode !== 'add') {
            console.error(`\u001b[91mMode supplied: ${argv.mode}. Must be 'sub' or 'add'. Please refer to README for more info.\u001b[0m`);
            failedFlag = true;
        } else {
            mode = tempMode;
        }
    ...
    if (argv.hasOwnProperty('scale')) {
        ...
        if (Number.isNaN(tempScale) || tempScale <= 0) {
            console.error(`\u001b[91mScale supplied: ${argv.scale}. Must be positive floating-point value. Please refer to README for more info.\u001b[0m`);
            failedFlag = true;
        } else {
    ...
    if (argv.hasOwnProperty('intp')) {
        if (!INTERPOLATIONS.hasOwnProperty(argv.intp.toLowerCase())) {
            console.error(`\u001b[91mInterpolation supplied: ${argv.intp}. Can be either: ${Object.keys(INTERPOLATIONS).join(", ")}. Please refer to README for more info.\u001b[0m`);
            failedFlag = true;
        } else {
    ...
    if (mode === 'add') {
        console.log(`\u001b[32mWatching for image file changes in ${path}\u001b[0m. Files will be appended with \u001b[33m'${suffix}'\u001b[0m. Press Ctrl+C to exit.`);
    } else {
        console.log(`\u001b[32mWatching for image file changes in ${path}\u001b[0m. Files with the suffix \u001b[33m'${suffix}'\u001b[0m will be saved without it. Press Ctrl+C to exit.`);
    }
            ...
            Jimp.read(name)
                .then(image => {
                    ...
                    if (match.length) {
                        ...
                        console.error(`\u001b[91mError in file ${name}: File does not match chosen file type (${EXTENSION})\u001b[0m`);
                    }
                })
                .catch(err => {
                    console.error(`\u001b[91mError in file ${name}: ${err}\u001b[0m`);
                });
        } else if (evt === "remove") {
